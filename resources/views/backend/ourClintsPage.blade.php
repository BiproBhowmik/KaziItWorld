						...
						<div id="datatable_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer"><div class="row"><div class="col-sm-12">
							...
							<thead>
								<tr role="row">
									...
									<th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 258px;">Our Clints Image</th>
									<th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 258px;">Our Clints Name</th>
									<th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 258px;">Our Clints Value</th>
							...
							<tbody>
								...
								<tr role="row" class="odd" id="commonId{{ $item->clId }}">
									<td>{{ $loop->index }}</td>
									<td><img width="30%" src="{{ Storage::url($item->clLogo)}}" alt="image"></td>
									<td>{{ $item->clName }}</td>
									<td>{{ $item->clVal }}</td>
									<td class="text-left py-0 align-middle">
										<div class="btn-group btn-group-sm text-left">
											<a onclick="editCommon({{ $item->clId }})" href="javascript:voud(0)" class="btn btn-warning"><i class="fa fa-pencil"></i></a>

											<meta name="csrf-token" content="{{ csrf_token() }}">  {{-- for Delete Ajax --}}
											<button class="btn btn-danger commonDelete" data-id="{{ $item->clId }}"><i class="fa fa-trash-o"></i></button>
										</div>
									</td>
								</tr>
								@endforeach
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form enctype="multipart/form-data" action="{{ route('addOurClints') }}" method="POST" id="commonAddForm">
					@csrf

					<div class="form-group">

							<label for="langName">Our Clints Logo</label>
					...
					<div class="card-body">
						...
						<div class="form-group">
							...
							<label for="langName">Our Clints Name</label>
							<input id="commonName" name="commonName"  type="text" class="form-control" placeholder="Enter A Service Name">
						</div>

						<div class="form-group">

							<label for="langName">Our Clints Value</label>
							<input id="commonValue" name="commonValue" type="text" class="form-control" placeholder="Our Clints Value">
						</div>
					...
					<div class="card-footer">
						<button type="submit" class="btn btn-primary">Add A Our Clints </button>
					</div>
...
<div class="modal fade" id="commonUpdateModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form enctype="multipart/form-data" action="{{ route('UpdateOurClint') }}" method="POST" id="updateCommonAddForm">
					@csrf

					<div class="form-group">

							<label for="langName">Our Clint Image</label>
					...
					<div class="card-body">
						...
						<div class="form-group">
							...
							<label for="langName">Our Clint Name</label>
						...
						<div class="form-group">
							...
							<label for="langName">Our Clint Value</label>
							<input id="updateCommonValue" name="updateCommonValue" type="text" class="form-control" placeholder="Enter A Value">
						</div>
					...
					<div class="card-footer">
						<button type="submit" class="btn btn-primary">Update Our Clint </button>
					</div>
...
@include('backend.myAjax.ajaxOurClint')
@endsection